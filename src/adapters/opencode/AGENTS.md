...
- **Tier 1 约束规范**：编码规则、模式、反模式。prompt 引用相关文件时插件自动注入完整约束；其他场景注入 **Spec Catalog**（spec 目录），由你按场景自行读取。
...
3. 收到 **Spec Catalog** 时，编码前先 Read 其中与当前任务匹配的 spec 全文（路径相对 `.code-flow/specs/`）；prompt 引用明确文件路径时插件自动注入完整约束，无需手动读取
