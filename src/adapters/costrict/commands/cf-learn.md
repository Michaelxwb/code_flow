...
自动扫描项目配置文件和代码模式，提取隐含的编码约束和团队规范，呈现给用户确认后写入 AGENTS.md 或 spec 文件。支持生成 Retrieval Map（导航地图）和基于当前工作区变更提炼新规范。
...
在进入代码扫描前，先构建 **统一排除集**：
- 默认排除：`.git/**`、`.code-flow/**`、`.costrict/**`、`.claude/**`、`.codex/**`、`.agents/**`、`.codex_flow/**`、`node_modules/**`、`dist/**`、`build/**`、`coverage/**`、`.next/**`、`.venv/**`、`venv/**`、`__pycache__/**`
...
- 全局规则写入 `AGENTS.md` 的 `## Core Principles` 或 `## Forbidden Patterns`
- 每条写入后输出确认
