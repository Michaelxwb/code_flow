...
与用户协作讨论 task.md 中的 `#NOTES` 标记，对齐最优解后将结论沉淀到任务中，删除标记。
...
- `/project:cf-task:note <file>` — 讨论指定文件中所有 `#NOTES`
- `/project:cf-task:note <file> TASK-001` — 只讨论指定子任务中的 `#NOTES`
...
可执行 /project:cf-task:start auth-module 开始编码。
```
