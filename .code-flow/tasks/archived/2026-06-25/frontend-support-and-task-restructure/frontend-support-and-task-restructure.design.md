...
| v1.1 | 2026-07-18 | Codex | 归档复核：schema-v1 初始化、真实 Agent E2E、前端注入覆盖与 active 启动顺序修正 |
...
|---------|--------|----------|-----------------|------|
| US-01 | FEAT-01 | cf-init/cf-learn patterns | TC-S05, TC-E01 | 已完成 |
| US-02 | FEAT-02 | design-frontend.md + align | TC-S06 | 已完成 |
| US-03 | FEAT-03 | cf-learn specs+checks | TC-S07, TC-E02 | 已完成 |
| US-04 | FEAT-04 | prd/align/plan 写路径 | TC-S01 | 已完成 |
| US-05 | FEAT-05 | align 后缀 design + plan 目录入参 | TC-S02, TC-S03, TC-B01 | 已完成 |
| US-06 | FEAT-06 | archive 整目录 | TC-S04, TC-E03 | 已完成 |
...
schema-v1 一次性迁移删除了旧 `inject.*` 配置和 `cf_session_hook.py`，但归档复核的真实 Codex Agent 走查发现，4 平台共 8 份 `cf-init` 指令仍要求补写 `inject.compress`、`inject.dedup_window` 与 `SessionStart`。Agent 因而追查已删除的 hook，未完成 Vue 动态 patterns 落盘。后续 live E2E 又发现 service/composable 路径未命中前端 Specs，以及 `cf-task-start` 先 active 后 refresh 会让 marker 自行漂移。本节把这些问题统一收口为归档前兼容性修正，不改变 FEAT-01~06 的产品语义。

### 7.2 修正约束

| ID | 约束 |
|----|------|
| CORR-01 | `cf-init` 只校准 schema-v1 字段：`spec_workflow.schema_version`、`spec_workflow.catalog.dedup_window`、`quality_loop.compress`；禁止恢复 `inject.*`。 |
| CORR-02 | Claude/Costrict/Codex 初始化指令以当前 adapter 模板为准，必须包含 `PreToolUse`、`PostToolUse`、`UserPromptSubmit`、`Stop`，不得要求已删除的 `SessionStart`。 |
| CORR-03 | Codex `hooks.json` 必须注册 `PreToolUse`，确保代码编辑前的 Context/scope fail-closed 检查真实生效。 |
| CORR-04 | 4 平台 canonical/deployed 指令保持 parity；CLI 仍为零外部依赖并保留用户配置。 |
| CORR-05 | 核心前端模板必须覆盖 `src/services/**` 与 `src/composables/**`，保证分层代码在编辑前命中前端 Specs。 |
| CORR-06 | `cf-task-start` 固定为 refresh/Start Gate → 读取新 Context hash → active start → session；禁止先 start 再 refresh。 |
| CORR-07 | 同一 artifact/section/item 的 Context application 必须幂等替换旧哈希并清理历史重复 ref，保证文档修订后可重新通过 Gate。 |

### 7.3 验收补充

| 场景ID | 层级 | 关键真实边界 | 预期结果 |
|--------|------|--------------|---------|
| S-05 | E2E | 真实 `codex exec` + 安装后的 `$cf-init` + Vue fixture | Agent 完成初始化，`frontend.patterns` 含 `**/*.vue`，无旧 `inject` 字段。 |
| E-01 | E2E | 真实 `codex exec` + 安装后的 `$cf-init` + React fixture | Agent 完成初始化，`frontend.patterns` 不含 `.vue`。 |
| S-06 | E2E | 真实 `codex exec` + 安装后的 `$cf-task-align` + 前端 PRD | 自动选择并产出 frontend design，包含前端模板核心章节。 |
| S-07 | E2E | 真实 `codex exec` + 安装后的 `$cf-learn` + 组件/services fixture | 输出证据化前端候选/checks，写入需经过确认门；service/composable 路径可命中 frontend 域。 |

真实 Agent 运行属于归档前的人工集成验收，不纳入默认 pytest；运行记录必须回填任务 Evidence，且不得把 Agent 的自然语言自述当作落盘断言。
