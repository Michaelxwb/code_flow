...
- 依赖: 零外部依赖（仅 fs/path/child_process）
...
├── mergeClaudeMd()        # CLAUDE.md 段落级合并
├── mergeSettingsJson()    # settings.json deep merge
├── mergeConfigYml()       # config.yml 顶层 key 合并
├── processDir()           # 目录递归复制（带分类策略）
└── runInit(force)         # 主流程: fresh / upgrade / current / force
```

## Data Flow

```
code-flow init → 检测模式(fresh/upgrade/current/force) → 按文件分类处理 → 写 .version → 输出摘要
```

## Navigation Guide

- 修改初始化行为 → `runInit()` 函数
- 新增文件分类 → `fileCategory()` 函数
- 修改合并策略 → `mergeClaudeMd()` / `mergeSettingsJson()` / `mergeConfigYml()`
- 新增 CLI 子命令 → args 解析区域（文件末尾）
