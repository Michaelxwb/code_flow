...
### Bug #1: 空 Spec-Refs 吞掉下一行

**问题**：`cf_spec_session.py` 的 `_refs()` 函数使用 `\s*(.+)` 正则，当 `Spec-Refs:` 为空时会跨行匹配下一字段。
...
**改动说明**：
- `\s*` → `[ \t]*`：只匹配空格/制表符（不跨行）
- `.+` → `.*`：允许匹配空字符串（空 Spec-Refs 合法）

**测试覆盖**：新增 `test_cf_spec_session_empty_refs.py`（5 个用例）
