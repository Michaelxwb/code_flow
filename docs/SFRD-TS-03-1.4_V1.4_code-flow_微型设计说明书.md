...
code-flow 是一套基于 **Skill + Python 脚本 + Claude Code Hook** 的 AI 辅助编码规范管理工具，解决小型团队（2-5 人）在版本迭代中的核心痛点：
...
**解决方案**：构建**工具无关的多层规范体系**（`.code-flow/specs/` 细分规范），通过适配层（Skill + Hook）对接不同 AI Code 工具，**自动感知编辑文件并按需注入**当前任务相关的规范子集。

**核心架构**：Skill → Python 脚本（via Bash）→ 文件系统；Hook → Python 脚本 → additionalContext 自动注入。

**支持的 AI Code 工具**（当前及规划）：
| 工具 | 适配方式 | 状态 |
|------|---------|------|
| Claude Code | Skill 文件（`.claude/skills/cf-*.md`）+ Hook（PreToolUse 自动注入） | MVP |
...
> **作为** 技术负责人，
> **我希望** 在 Claude Code 中运行 `/cf-init`，一键检测技术栈并生成完整的规范体系（目录、模板、Skill 文件、Python 脚本、Hook 配置、L0 配置），仅需 Python 3.9+ 和 pyyaml，
> **以便** 团队成员 clone 后 5 分钟内可开始 AI 辅助开发。
>
> **验收标准**：
> - 自动检测前端/后端技术栈（如 React + FastAPI）
> - 一次性生成：CLAUDE.md + `.code-flow/` 全套目录（含 scripts/）+ `.claude/skills/cf-*.md` 全部 Skill 文件 + `.claude/settings.local.json` Hook 配置
...
> **作为** 前端开发者，
> **我希望** 在编辑 `src/components/UserAvatar.tsx` 时，**无需手动执行任何命令**，AI 自动通过 Hook 加载组件规范和前端质量准则，而不是后端 DB 规范，
> **以便** AI 生成的代码严格遵循前端团队的目录结构、命名约定和组件模式。
>
> **验收标准**：
> - 编辑代码文件时 Hook 自动触发，无需手动执行 `/cf-inject`
> - 仅加载 `frontend/component-specs.md` 和 `frontend/quality-standards.md`
> - 不加载 `backend/*.md` 的任何内容
> - 总注入 token ≤ 2500
> - 同一领域在同一 session 内仅注入一次（去重）
...
> **作为** 团队成员，
> **我希望** 在发现 AI 的常见错误模式后，通过 `/cf-learn` 将经验**直接写入对应的 spec 文件**，
> **以便** 下次注入该 spec 时自动包含新学到的经验，规范越来越精准。
>
> **验收标准**：
> - 交互时列出匹配领域的所有 spec 文件，由用户选择写入哪个
> - 学习内容追加到选定 spec 文件末尾的 `## Learnings` 段落
> - 全局学习写入 CLAUDE.md 的 `## Learnings` 段落
> - 下次 Hook 自动注入该 spec 时自动包含 Learnings 内容
...
| 适配层 | 将 code-flow 统一规范转换为特定工具格式的桥接层（Skill / Hook / .cursorrules / AGENTS.md） |
| Skill | Claude Code 的扩展机制，markdown 指令文件，用户通过 `/skill-name` 调用 |
...
| L0 / L1 | 上下文层级：L0=全局原则（工具原生配置）→ L1=领域规范（specs/ 含内嵌 Learnings） |
...
**Skill + Python + Hook 架构**：

- **适配层**：Claude Code Skill 文件（`.claude/skills/cf-*.md`）+ Claude Code Hook（PreToolUse 自动注入）
- **计算层**：Python 3.9+ 脚本（`.code-flow/scripts/`），处理配置解析、路径匹配、token 估算、去重状态管理
...
graph TB
    ...
    subgraph Adapter["适配层"]
        A1[Claude Code Skills<br/>.claude/skills/cf-*.md]
...
|-------------|---------|------------|---------|------|
| Claude Code | Skill + Hook（PreToolUse） | CLAUDE.md | Hook 自动注入 + Skill 手动回退 | MVP |
...
| 维度 | 纯 Skill（V1.3） | Skill + Python（V1.4，当前方案） |
|------|-----------------|-------------------------------|
| Hook 支持 | **不支持** — Skill 无法作为 Hook 脚本执行 | **原生支持** — Python 脚本可作为 Hook command |
| 自动注入 | 不支持，必须手动 `/cf-inject` | **PreToolUse Hook 自动注入**，编辑代码文件时零操作 |
| 执行速度 | Claude 调用多个原生工具，耗时数秒 | Python 脚本直接读取文件，毫秒级响应 |
| 配置解析 | Claude 理解 YAML 语义（不可靠） | pyyaml 精确解析，逻辑确定性 |
| 安装复杂度 | 零依赖 | 需 Python 3.9+ 和 `pip install pyyaml` |
| Token 计数 | Claude 内建估算 | `len(text) // 4`（相同精度，确定性执行） |
| 维护成本 | 仅 Skill markdown | Skill markdown + 5 个 Python 文件 |
...
│
├── .claude/
│   ├── skills/                            # 适配层: Claude Code Skill（MVP）
│   │   ├── cf-init.md
│   │   ├── cf-scan.md
│   │   ├── cf-inject.md
│   │   ├── cf-validate.md
...
L1 (specs/frontend/*.md)  — 编辑前端文件时通过 Hook 自动注入
L1 (specs/backend/*.md)   — 编辑后端文件时通过 Hook 自动注入
                            内容：领域特定的编码约束 + 内嵌 Learnings 段落
...
**注入触发方式**：
- **自动**（主要）：Hook 在 Edit/Write/MultiEdit 调用前自动触发
- **手动**（回退）：`/cf-inject` 命令，用于 Hook 未触发或需要强制重新注入的场景

**Learnings 内嵌**：学习记录直接写在 spec 文件末尾的 `## Learnings` 段落中，注入 spec 时自动包含，无需独立文件。
...
一键检测项目技术栈，生成完整的规范体系：L0 配置文件 + `.code-flow/` 全套目录（含 Python 脚本）+ Skill 文件 + Hook 配置。**依赖 Python 3.9+ 和 pyyaml**。
...
2. 用 Write 工具批量生成文件（顺序）：
   ...
   k. .claude/skills/cf-*.md（6 个 Skill 文件）
...
## Spec Loading
本项目使用 code-flow 分层规范体系。
按需规范位于 .code-flow/specs/ 目录下。
- Claude Code: 编辑代码文件时通过 Hook 自动注入；也可通过 /cf-inject 手动加载
...
| Python 3.9+ 不可用 | 输出错误提示，建议安装 Python 3.9+；Skill 文件仍然生成（手动模式可用） |
...
`.code-flow/scripts/` 下的 Python 脚本构成计算层，为 Skill 命令和 Hook 自动注入提供确定性的配置解析、路径匹配和 token 估算能力。
...
# 被 /cf-scan Skill 通过 Bash 调用: python3 .code-flow/scripts/cf_scan.py [project_root]
# 输出 JSON 到 stdout
...
# 被 /cf-stats Skill 通过 Bash 调用: python3 .code-flow/scripts/cf_stats.py [project_root]
# 输出 JSON 到 stdout
...
**手动回退方式**。当 Hook 自动注入未触发或需要强制重新注入时，用户可手动执行 `/cf-inject` 加载规范。V1.4 中此命令降级为回退机制，主要注入方式为 Hook 自动注入（见 3.4）。

**典型使用场景**：
- Hook 未配置或 Python 不可用
- 需要在对话开始时预加载规范（尚未编辑任何文件）
- 需要强制重新注入（清除去重状态）
...
Skill 在执行 Bash 命令前，对 `{files}` 中的每个文件路径用单引号包裹，防止 shell 注入。仅接受 `git diff` 输出的文件路径。
...
3. Skill 将 JSON 结果格式化为人类可读的统计表格：
...
将团队学习到的经验**直接写入对应的 spec 文件**末尾的 `## Learnings` 段落，下次注入该 spec 时自动包含。
...
2. 确定写入目标：
   - 全局 → CLAUDE.md 的 ## Learnings 段落
   - 前端/后端 → 列出该领域下所有 spec 文件供用户选择
     例: "请选择写入哪个规范文件："
     1. frontend/component-specs.md
     2. frontend/quality-standards.md
     3. frontend/directory-structure.md

3. 用 Read 读取目标文件，定位 ## Learnings 段落
   - 如段落不存在 → 在文件末尾追加 ## Learnings 标题

4. 用 Edit 在 ## Learnings 段落末尾追加：
   - [YYYY-MM-DD] 学习内容

5. 输出确认 + 更新后的 token 变化
```

### 3.9.3. 写入示例

用户执行 `/cf-learn` 输入"组件 Props 必须定义 interface，禁止 inline type"，选择 `frontend/component-specs.md`，结果：
...
## Learnings
- [2026-03-10] 旧的学习内容...
- [2026-03-13] 组件 Props 必须定义 interface，禁止 inline type
```
...
| 场景 | 处理策略 |
|------|---------|
| 目标 spec 文件不存在 | 提示运行 `/cf-init` 或手动创建 |
| Learnings 段落不存在 | 自动在文件末尾追加 `## Learnings` 标题 |
...
## 4.2. Hook 与 Skill 的交互

- **Hook 优先**：编辑代码文件时 Hook 自动触发注入，开发者无需手动操作
- **Skill 回退**：Hook 未触发（Python 不可用 / 非编辑操作 / 配置缺失）时，用户可手动 `/cf-inject`
- **去重协同**：`/cf-inject` 执行后同样更新 `.inject-state`，防止 Hook 重复注入已手动加载的领域
- **状态清理**：SessionStart Hook 清理 `.inject-state`，确保每个新会话都能获得完整的规范注入

## 4.3. 安全性

- `/cf-validate` 执行用户定义的 shell 命令 — Skill 对文件路径用单引号包裹防注入
