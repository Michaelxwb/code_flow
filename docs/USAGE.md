...
同一规则误报 ≥3 次或误报率 >10% 自动停用并在 `cf-stats` 标注；`cf-scan` 输出待复审清单（长期未命中 / 已停用 / 反复误报的规范），处置后加入 `.check-state.json` 的 `_review_exempt` 列表即不再提示。
...
- 约束 spec 缺 frontmatter `description` → 标记"缺描述"（Spec Catalog 行来源，v0.5+）
- `checks` 标注语法错误 / message 超长 → 标记"checks"（v0.5）

**v0.5 新增输出**：末尾 `CATALOG:`（目录 token/条目数）与 `REVIEW (待复审):` 清单——长期未命中注入、已自动停用、反复误报的规范条目（处置后加入 `.check-state.json` 的 `_review_exempt` 不再提示）。
