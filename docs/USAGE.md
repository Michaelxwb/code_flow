...
|---------|---------|--------------|------------|
| [Claude Code](https://claude.ai/code) | `CLAUDE.md` | 编辑文件前（`PreToolUse`）+ 提交 prompt 时（`UserPromptSubmit`） | `/cf-init`、`/cf-learn` 等 |
...
| [OpenCode](https://opencode.ai) | `AGENTS.md` | 提交消息时（`chat.message` plugin 事件） | `/cf-init`、`/cf-learn` 等 |
...
│   cf-init.md、cf-learn.md、cf-scan.md、cf-task/{prd,align,plan,...}.md
...
Claude Code 同时支持 `UserPromptSubmit` Hook，提交 prompt 即注入。
...
Codex CLI、Costrict 和 OpenCode 的项目级 AI 指令文件，每次对话自动加载。结构与 `CLAUDE.md` 相同，`Spec Loading` 节说明由 Hook / Plugin 注入约束规范（Codex 使用 `UserPromptSubmit Hook`，Costrict 使用 `PreToolUse Hook`，OpenCode 使用 `chat.message` Plugin 事件）。
...
| `chat.message` | 用户提交消息 | `cf_user_prompt_hook.py` | 从消息提取文件引用与关键词，缓存匹配的 specs |
| `experimental.chat.system.transform` | 系统 prompt 组装阶段 | （插件内联） | 把缓存的 spec context push 到 system prompt |
